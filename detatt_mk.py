...
def tls(x,y,cn1,cn2='None',ne=1,rof_flag=1,CI_flag=1,alpha=0.10,flag_2S=0,flag_3S=0,flag_4S=0,error_flag=0):
	...
	if cn2 is 'None':
		...
		if CI_flag==1:
			if error_flag==1:
				return (beta,betaCI,rc_pvalue,rc_mcCI,err)
			else:
				return (beta,betaCI,rc_pvalue,rc_mcCI)
		else:
			if error_flag==1:
				return (beta,rc_pvalue,rc_mcCI,err)
			else:
				return (beta,rc_pvalue,rc_mcCI)
...
def adjust_beta(xa,y,cn1,xn='None',weights='None'):
	...
	#fit scaling factors
	beta1,err1 = tls(ema1[:,None],y1,cn1,ne=ne2,CI_flag=0,error_flag=1)
	if xn is not 'None':	
		beta2,err2 = tls(np.vstack((ema1,emn1)).T,y1,cn1,ne=ne2,CI_flag=0,flag_2S=1,error_flag=1)
...
def att_trend(emA,y,cn1,cn2,ne,yrs,emN='None',start_year='None',end_year='None',o_flag=0):
	...
	#fit scaling factors
	beta1,beta1CI,tmp,tmp,err1 = tls(ema1,y1,cn1,cn2,ne=ne,CI_flag=1,error_flag=1)
	if emN is not 'None':	
		beta2,beta2CI,tmp,tmp,err2 = tls(np.hstack((ema1,emn1)),y1,cn1,cn2,ne=ne,CI_flag=1,flag_2S=1,error_flag=1)
